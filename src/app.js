...
import { Modal, WarriorForm, Warriors, Stats, MapAddForm } from '_blocks'
import { premissionRequest } from './config/roles'
import resetSVG from 'assets/icons/spinner.svg'
import { ICONSIZE, ICONRESIZESTEP, SHIFTYAXIS } from 'constants/index'
...
export default class App extends PureComponent {
  ...
  calculatePosition = () => {
    ...
    const positionedWarriors = mappedWarriors.map((warrior) => {
      ...
      const isInlatRange =
        +warrior.lat > +map.south && +warrior.lat < +map.north
  ...
  render() {
    ...
    const modalConditional = modal.show ? (
      <Modal data={modal} show={modal.show} onHide={this.modalHide} />
    ) : null
        ...
        <div style={{ height: SHIFTYAXIS }} className="app__header">
          <div className="app__header-lable">
            ...
            <B.ButtonGroup className="app__header-button-group app__header-buttons">
              <B.Button
                className="app__form-button-long"
                onClick={() => this.resize('+')}
              >
                +
              </B.Button>
              <B.Button onClick={this.resize}>
                <img
                  className="app__header-reset-button"
                  alt="reset"
                  src={resetSVG}
                />
              </B.Button>
